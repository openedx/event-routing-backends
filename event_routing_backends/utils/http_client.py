...
class HttpClient:
    ...
    def send(self, event, event_name):
        ...
        logger.info('Sending caliper version of edx event "{}" to {}'.format(event_name, self.URL))
        response = requests.post(**options)

        if not 200 <= response.status_code < 300:
            logger.warning('{} request failed for sending Caliper version of edx event "{}" to {}. '
